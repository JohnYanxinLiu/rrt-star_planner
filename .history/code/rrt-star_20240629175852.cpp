...
// #define GOAL_BIAS_RATE 0.2;
static const double GOAL_BIAS_RATE = 0.1;

data_t random_sample(data_t q_goal, int x_size, int y_size, int numofDOFs) {
    data_t q_rand = init_data(numofDOFs);
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<double> dis(0.0, 1.0);
    
    if (dis(gen) < GOAL_BIAS_RATE) {
        // Bias towards goal configuration
        return q_goal;
    }
    ...
    while (1){
        for (int i = 0; i < numofDOFs; i++){
            {
                // Sample uniformly within the joint limits
                double lowerLimit = 0.0;
                double upperLimit = 2 * PI;
                q_rand[i] = lowerLimit + dis(gen) * (upperLimit - lowerLimit);
            }
            
            q_rand[i] = (static_cast<double>(rand()) / RAND_MAX) * (2 * PI);
        }
        vector<double> pos = manipulatorPosition(q_rand, x_size);
        ...
        int* planlength)
{
    ...
    while (num_samples < K_SAMPLES) {
        ...
        data_t q_rand = random_sample(q_goal, numofDOFs, x_size, y_size);
