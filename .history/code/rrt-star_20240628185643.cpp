...
data_t random_sample(double** map, int x_size, int y_size, int numofDOFs) {
    data_t q_rand = init_data(numofDOFs);
    
    while (1){
        for (int i = 0; i < numofDOFs; i++){
            angles[i] = (double) ((rand()%(359 + 1)) * (PI / 180));
        }

        if (IsValidArmConfiguration(angles, numofDOFs, map, x_size, y_size)){
            // for (int i = 0; i < numofDOFs; i++){
            //     printf("%d(%f) | ", i, angles[i]);
            //     fflush(stdout);
            // }
            // printf("\n");
            // fflush(stdout);
            State *s = new State(angles);
            printf("new state: %s\n", s->getStateKey(numofDOFs).c_str()); 
            return s;
        }
    }
}
