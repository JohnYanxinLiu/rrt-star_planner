...
static const double GOAL_BIAS_RATE = 0.2;
    ...
    while (1){
        for (int i = 0; i < numofDOFs; i++){
            if (dis(gen) < GOAL_BIAS_RATE) {
                ...
                q_rand[i] = goalConfiguration[i];
            } else {
                // Sample uniformly within the joint limits
                // Example: Assuming joint limits are [0, 2π] radians
                double lowerLimit = 0.0;
                double upperLimit = 2 * M_PI;
                sampledAngles[i] = lowerLimit + dis(gen) * (upperLimit - lowerLimit);
            }
