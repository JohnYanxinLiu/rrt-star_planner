...
list<data_t> Graph::k_nearest_neighbors(data_t q, size_t num_neighbors)
{
    ...
    {
        neighbors.push_front(pq.top().second);
        pq.pop();
    }
...
GOAL_STATUS extend(Graph *G, data_t q_rand, data_t q_goal, int numofDOFs, double* map, int x_size, int y_size) {
    ...
    if (collision_free(q_nearest, q_new, numofDOFs, map, x_size, y_size)) {
        ...
        list<data_t> neighbors = graph.k_nearest_neighbors(q_new, K_NEIGHBORS);
        ...
        {
            //print out q_neighbor
            // cout << "q_neighbor: ";
            // for (int i = 0; i < q_neighbor.size(); i++)
            // {
            //     cout << q_neighbor[i] << " ";
            // }
            // cout << endl;
            ...
            // cout << "Old cost: " << graph.cost(q_new) << " | New cost: " << new_cost << endl;
        ...
        int* planlength)
{
    ...
    q_test2[1] += 0.2;
    graph.add_node(q_test1, 0.1);
    graph.add_node(q_test2, 0.2);
    
    data_t q_nearest_test = graph.nearest_neighbor(q_start);
    cout << "Nearest neighbor to start: ";
    for (int i = 0; i < q_nearest_test.size(); i++)
    {
        cout << q_nearest_test[i] << " ";
    }
    cout << endl;

    // data_t q_test_steer = graph.steer(q_start, q_test);
    // cout << edge_cost(q_test, q_test_steer) << endl;
    // cout << edge_cost(q_start, q_test_steer) << endl;
    // cout << "q_test: ";
    ...
    // cout << "q_test_steer: ";
    // for (int i = 0; i < q_test_steer.size(); i++)
    // {
    //     cout << q_test_steer[i] << " ";
    // }
    // cout << endl;
